...
const DefaultPagination = ({hasPrevious, hasNext, totalPages, currentPage, EventClickSwitchPage, totalCount}:Props) => {
  let start:number=0;
  let end:number=0;
  ...
  if(totalPages!=0){
    if(currentPage<totalPages-4){
      ...
      start=totalPages-4<1?1:totalPages-4;
      end=totalPages-1;
    }
  }

  let arrPages:number[]=totalPages==0?[]:createArrayFromRange(start,end);
  if(arrPages.some((item:any)=>item<0)){
    arrPages=[];
  }
  ...
  return (
    ...
    <nav className='flex flex-row'>
      ...
      <div className="flex justify-center items-center gap-x-3 w-10/12" aria-label="Pagination">
        ...
        gap-x-2 text-sm rounded-lg border border-cyan-400 text-gray-800 hover:bg-cyan-400 hover:text-white focus:outline-none disabled:opacity-50 disabled:pointer-events-none" aria-label="Previous">
      ...
      <div className='w-1/12 flex justify-center items-center font-semibold text-base text-gray-400'>
        Tổng: {totalCount}
      </div>
    </nav>
  )
